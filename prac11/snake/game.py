...
FOOD_SPAWN = pygame.USEREVENT + 1  # ИЗМЕНЕНО


def draw_grid():
    for row in range(PLAY_ROWS):
        for col in range(GRID_COLS):
            pygame.draw.rect(screen, colorGRAY, (col * CELL, row * CELL, CELL, CELL), 1)
...
def draw_overlay(lines, color=(255, 255, 255)):
...
class Snake:
    def __init__(self):
        mid = PLAY_ROWS // 2
        self.body = [Point(10, mid), Point(9, mid), Point(8, mid)]
        self.dx = 1
        self.dy = 0
        self.alive = True

    def set_direction(self, dx, dy):
        if dx == -self.dx and dy == -self.dy: return
    ...
    def check_food(self, food):
        head = self.body[0]
        # ИЗМЕНЕНО: теперь проверяем только если еда существует
        if food.exists and head.x == food.pos.x and head.y == food.pos.y:
            self.grow()
            return True
        return False
    ...
    def draw(self):
        head = self.body[0]
        pygame.draw.rect(screen, colorRED, (head.x * CELL + 1, head.y * CELL + 1, CELL - 2, CELL - 2))
        for seg in self.body[1:]:
            pygame.draw.rect(screen, colorYELLOW, (seg.x * CELL + 2, seg.y * CELL + 2, CELL - 4, CELL - 4))


class Food:
    def __init__(self):
        self.pos = Point(15, 10)
        self.exists = False  # ИЗМЕНЕНО: добавили флаг существования

    def generate_random_pos(self, snake):
        while True:
            x, y = random.randint(0, GRID_COLS - 1), random.randint(0, PLAY_ROWS - 1)
    ...
    def draw(self):
        if self.exists:
            pygame.draw.rect(screen, colorGREEN, (self.pos.x * CELL + 2, self.pos.y * CELL + 2, CELL - 4, CELL - 4))
...
def reset_game():
    ...
    food.exists = False
    pygame.time.set_timer(FOOD_SPAWN, random.randint(1000, 3000))
...
while running:
    for event in pygame.event.get():
        ...
        # ИЗМЕНЕНО: Логика появления еды по таймеру
        if event.type == FOOD_SPAWN and not game_over:
            food.exists = False
            food.generate_random_pos(snake)
            food.exists = True
            pygame.time.set_timer(FOOD_SPAWN, random.randint(5000, 20000))
    ...
    if not game_over:
        ...
        if not snake.alive:
            ...
            foods_eaten += random.randint(1,4)
            score += foods_eaten
            food.exists = False
    ...
    if level_flash > 0 and not game_over:
        draw_overlay([f"LEVEL {level}!", f"Speed ×{level}"], colorORANGE)
    if game_over:
        draw_overlay(["GAME OVER", f"Score: {score}", "ENTER to restart"], colorRED)
