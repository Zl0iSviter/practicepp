...
WIDTH  = 600
HEIGHT = 660        # extra 60 px at the bottom for the HUD
CELL   = 30
...
class Snake:
    def __init__(self):
        # Start in the middle of the play area, moving right
        mid = PLAY_ROWS // 2
        ...
        self.dx = 1
        self.dy = 0
        self.alive = True       # False once the snake hits a wall or itself
    ...
    def check_food(self, food: "Food") -> bool:
        """Return True (and grow) if the head overlaps the food."""
        head = self.body[0]
    ...
    def draw(self):
        """Draw head in red, body segments in yellow."""
        head = self.body[0]
        pygame.draw.rect(
            screen, colorRED,
...
class Food:
    ...
    def generate_random_pos(self, snake: Snake):
        """
        Place food at a random grid cell that is:
...
# Game reset
def reset_game():
    """Return fresh snake, food, score, level, fps and foods-eaten counter."""
    snake = Snake()
    food  = Food()
...
game_over   = False
level_up    = False
level_flash = 0     # countdown timer (frames) for the level-up banner
...
while running:
        ...
        if not snake.alive:
            ...
            if snake.check_food(food):
                ...
                if foods_eaten >= FOODS_PER_LEVEL:
                    level       += 1
                    foods_eaten  = 0
                    current_fps += SPEED_INCREMENT   # speed up
                    level_flash  = int(current_fps * 1.5)  # show banner

        # Tick the level-up banner countdown
        if level_flash > 0:
            level_flash -= 1
    ...
    pygame.display.flip()
    clock.tick(current_fps)   # frame rate changes with level
