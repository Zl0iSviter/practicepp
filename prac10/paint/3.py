...
def main():
    ...
    # tool can be: 'pencil', 'rect', 'circle', 'eraser'
    tool = 'pencil'
    ...
    # for rect and circle: store the start point when mouse is pressed
    shape_start = None

    # a separate surface we draw on permanently
    canvas = pygame.Surface((640, 480))
    canvas.fill((0, 0, 0))

    # color palette: list of (rect, color) for clickable swatches at the bottom
    palette_colors = [
        (255, 0,   0),
        (0,   255, 0),
        (0,   0,   255),
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                # right click shrinks radius (kept from original)
                if event.button == 3:
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                if event.button == 1:
                    # commit rect or circle to canvas on mouse release
        ...
        # draw palette swatches at the bottom
        for i, rect in enumerate(palette_rects):
            pygame.draw.rect(screen, palette_colors[i], rect)
            if palette_colors[i] == color:
                pygame.draw.rect(screen, (255, 255, 255), rect, 2)

        # draw live preview of rect/circle while dragging
